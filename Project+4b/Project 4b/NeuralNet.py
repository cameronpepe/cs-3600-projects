...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """
        """YOUR CODE"""
        inActsBiased = [1.0] + inActs
        wx = self.getWeightedSum(inActsBiased)
        return self.sigmoid(wx)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        g = self.sigmoidActivation(inActs)
        return g * (1 - g)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            layerActs = []
            for percep in layer:
                layerActs.append(percep.sigmoidActivation(acts[-1]))
            acts.append(layerActs)
        return acts
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[-2]) #"""FILL IN func param"""
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum]) #"""FILL IN func param"""
                    ...
                    for nextNeuronNum in xrange(len(nextLayer)):
                        nextLayerError += nextLayer[nextNeuronNum].weights[neuronNum + 1] * deltas[0][nextNeuronNum]
                    delta = gPrime * nextLayerError #layer[neuronNum].sigmoidActivation(deltas[0])
            ...
            for numLayer in xrange(0,self.numLayers):
                ...
                for numNeuron in xrange(len(layer)):
                    weightMod = layer[numNeuron].updateWeights(allLayerOutput[numLayer], alpha, deltas[numLayer][numNeuron]) # """FILL IN func params"""
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
	...
	For each text example, you should first feedforward to get the NN outputs. 
	Then, round the list of outputs from the output layer of the neural net.
	If the entire rounded list from the NN matches with the known list from the 
	test example, then add to testCorrect, else add to  testError.
    """
    ...
    for exampleIn, exampleOut in examplesTest:
        nnetOutputs = nnet.feedForward(exampleIn)[-1]
        outputs = [round(output) for output in nnetOutputs]
        if not cmp(outputs, exampleOut):
            testCorrect += 1
        else:
            testError += 1

    testAccuracy = (testCorrect * 1.0) / ((testCorrect + testError) * 1.0)
