...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostDist in livingGhostPositionDistributions:
            maxPos = ghostDist.argMax()
            maxBeliefs.append((maxPos, ghostDist[maxPos]))

        bestAction = (None, float("inf"))
        for a in legal:
            for ghost in maxBeliefs:
                successor = Actions.getSuccessor(gameState.getPacmanPosition(), a)
                actionDistance = self.distancer.getDistance(successor, ghost[0])
                if actionDistance < bestAction[1]:
                    bestAction = (a, actionDistance)
